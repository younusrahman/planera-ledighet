...
interface AppDndContextProps {
  ...
  onAbsenceEdit: (id: string) => void;
  onAbsenceDelete: (id: string) => void;
  onAbsenceResizeEnd: (
    id: string,
    newDuration: number,
    daysShifted: number,
...
export const AppDndContext = forwardRef<HTMLDivElement, AppDndContextProps>(
  (props, ref) => {
    const {
      days,
      daysCount,
    ...
    // 1. Beräkna vilka Absence som faktiskt är inom det synliga fönstret
    const visibleAbsences = useMemo(() => {
      ...
      return absences.filter((l) => {
        const absenceStart = dayjs(l.startDate);
        // Vi räknar ut blockets slutdatum
        const absenceEnd = absenceStart.add(l.durationDays, "day");

        // Ett block ska visas om:
        // Blocket startar INNAN tidslinjen slutar...
        // OCH blocket slutar EFTER att tidslinjen börjar.
        return (
          absenceStart.isBefore(timelineEndDate) &&
          absenceEnd.isAfter(startDate)
        );
      });
                      ...
                      <Collapse in={!isCollapsed}>
                        {(group.resources || []).map((res) => (
                          ...
                          >
                            ...
                            {/* Render Absence Blocks for this Resource */}
                            ...
                            {visibleAbsences
                              ...
                              .map((l) => (
                                <AbsenceBlock
                                  ...
                                  key={l.id}
                                  absence={l}
                                  onResizeEnd={onAbsenceResizeEnd}
                                  resourceName={res.name}
                                  left={getDateOffset(l.startDate, startDate)}
                                  onEdit={onAbsenceEdit}
              ...
              <DragOverlay adjustScale={false}>
                {activeAbsence && (
                  <AbsenceBlock
                    ...
                    left={0}
                    absenceTypes={absenceTypes.find(
                      (t) => t.id === activeAbsence.absenceTypeId,
                    )}
                    onResizeEnd={onAbsenceResizeEnd}
                    onEdit={onAbsenceEdit}
                    onDelete={onAbsenceDelete}
